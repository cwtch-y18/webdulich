...
if ($("#myDropzone").length) {
    ...
    $(".add-tours #wizard .buttonNext").on("click", function (event) {
        ...
        if (currentStep === 2) {
            ...
            let totalFiles = myDropzone.files.length; // ✅ ĐÚNG

            if (totalFiles < 5) {
                toastr.warning("Vui lòng thêm đủ 5 hình ảnh trước khi tiếp tục.");
                finishStep2 = false;
                return;
            }

            // Nếu còn file chưa upload → upload
            if (myDropzone.getQueuedFiles().length > 0) {
                myDropzone.processQueue();
            } else {
                // Đã upload đủ rồi → cho qua step 3
                finishStep2 = true;
                $(".add-tours #wizard").smartWizard("next");
            }
        }
    });
    ...
    myDropzone.on("queuecomplete", function () {
        if (myDropzone.files.length >= 5) {
            finishStep2 = true;
            toastr.success("Đã tải đủ 5 ảnh, có thể tiếp tục.");
        } else {
            finishStep2 = false;
        }
    });

    // ❗ XÓA ẢNH → BẮT BUỘC UP LẠI
